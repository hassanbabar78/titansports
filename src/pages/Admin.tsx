...
import { Textarea } from '@/components/ui/textarea';
import { toast } from 'sonner';
import { Pencil, Trash2, Plus, Package, DollarSign, ShoppingCart, MessageSquare } from 'lucide-react';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { Navigate } from 'react-router-dom';
import { Loader2 } from 'lucide-react';
import { Skeleton } from '@/components/ui/skeleton';
  ...
  return (
    <div className="min-h-screen bg-primary text-primary-foreground">
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Messages */}
        <h2 className="text-xl font-bold uppercase mt-12 mb-6 tracking-wide">
          Contact <span className="text-gold">Messages</span>
        </h2>
        <div className="bg-primary-foreground/5 border border-primary-foreground/10 rounded-xl overflow-x-auto animate-fade-in">
          {messages.length === 0 ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {messages.map((m: any) => (
                  <tr
                    key={m.id}
                    onClick={() => { if (m.status === 'unread') markReadMutation.mutate(m.id); }}
                  ...
                  >
                    ...
                    <td className="p-4">
                      {m.status === 'unread'
                        ? <span className="bg-gold/20 text-gold text-xs px-2 py-1 rounded-full">Unread</span>
                        : <span className="bg-primary-foreground/10 text-primary-foreground/50 text-xs px-2 py-1 rounded-full">Read</span>
                      }
                    </td>
                  </tr>
                ))}
