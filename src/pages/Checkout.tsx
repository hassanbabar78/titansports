        ...
        <div className="grid md:grid-cols-5 gap-8">
          <div className="md:col-span-3 space-y-6">
            ...
            <div className="bg-card p-6 rounded-lg border">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-bold uppercase text-sm">Payment</h2>
                <span className="text-xs bg-blue-500/20 text-blue-400 px-2 py-1 rounded font-semibold">PAYPAL</span>
              </div>
              <PayPalScriptProvider options={{ clientId: import.meta.env.VITE_PAYPAL_CLIENT_ID, currency: 'USD' }}>
                <PayPalButtons
                  ...
                  disabled={processing}
                  createOrder={async () => {
                    if (!user) { toast.error('Please sign in'); navigate('/login'); throw new Error('Not logged in'); }
                    ...
                    const { data, error } = await supabase.functions.invoke('create-paypal-order', {
                      body: {
                        items: items.map(i => ({ product_id: i.id, quantity: i.quantity, weight: i.weight })),
                        shipping_address: { firstName: form.firstName, lastName: form.lastName, email: form.email, address: form.address, city: form.city, state: form.state, zip: form.zip, country: form.country },
                        shipping_method: shipping,
                      },
                    });
                    if (error || !data?.paypal_order_id) throw new Error(data?.error || 'Failed to create order');
                  ...
                  onApprove={async (data) => {
                    ...
                    try {
                      ...
                      if (error || !result?.order_id) throw new Error(result?.error || 'Payment failed');
                      clearCart();
                      toast.success('Order placed successfully!');
              ...
              <p className="text-xs text-muted-foreground mt-3 flex items-center gap-1">
                <Shield className="h-3 w-3" /> Payments secured by PayPal
              </p>
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            <div className="bg-card p-6 rounded-lg border sticky top-24">
              ...
              <Link to="/shop" className="block text-center text-sm text-gold mt-4 hover:underline">← Back to Shop</Link>
            </div>
          </div>
        </div>
      </div>
